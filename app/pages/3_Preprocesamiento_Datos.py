...
* Por un lado hay que tener en cuenta que se está trabajando con datos correspondientes a distintos países y por tanto los rangos asociados a los datos macroeconómicos (a pesar de haber usado parámetros estandarizadores), podrían ser diferentes sin que eso supusiera la existencia de outliers. En este caso al menos se deberían buscar valores atípicos limitando los datos a los correspondientes a cada país en lugar de hacer una búsqueda sobre el conjunto completo de datos disponibles.

* Sin embargo el punto más grave de proceder con la realización de una eliminación de outliers sería el siguiente: como se sabe uno de los posibles indicadores predictivos de **:orange[una posible situación futura de crisis financiera es un escenario de hiper-inflación. Este escenario se traduciría en un valor atípico del indicador macroeconómico del Índice de Precios al Consumo. Si como parte del proceso de procesamiento de los datos se eliminan estos valores atípicos, se estaría evitando que el modelo predictivo pudiera detectar este tipo de patrones]** para la predicción de las futuras crisis financieras. Ésto mismos podría ocurrir con el resto de atributos empleados.</div>''', unsafe_allow_html=True)
