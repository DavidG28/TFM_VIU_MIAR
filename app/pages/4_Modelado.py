...
Dado el desbalance en lo que a número de muestras asociadas a cada clase respecta, es necesario aplicar algún tipo de estrategia para conseguir un número equitativo de muestras en cada clase. Para ello se han aplicado las siguientes metodologías a cada uno de los algoritmos indicados en el punto anterior:''')
    with st.container():
        ...
        with col1:
            st.write('''**:orange[SMOTE (Syntetic Minority Oversampling Technique):]** se trata de una técnica que se encarga de crear muestras artificiales de la clase minoritaria a partir de las muestras existentes en el conjunto de datos que toma como origen.
            
El funcionamiento se puede ver de manera gráfica en la siguiente imagen:''')
    
        with col2:
            st.write('''**:orange[Random Oversampling:]** se trata de nuevo de una técnica de “sobremuestreo” u “oversampling” de la clase minoritaria, pero en este caso no se crean muestras artificiales, sino que de la partición original se eligen de forma aleatoria muestras existentes correspondientes a dicha clase que se incluyen repetidas veces en la partición final para conseguir el deseado balance en el número de muestras.''')
    
        with col3:
            st.write('''**:orange[Random Undersampling:]** a diferencia de los dos casos anteriores, esta técnica consiste en un “submuestreo” o “undersampling” de las muestras asociadas a la clase mayoritaria, manteniendo inalterado el número de muestras correspondientes a la clase minoritaria. Como consecuencia se reduce enórmemente el número de muestras útiles.''')
        
        with col4:
            st.write('''**:orange[Balance de pesos:]** se ha hecho uso del parámetro “class_weight”, que permite calibrar los pesos asociados a cada clase, de forma que el algoritmo pondere de manera diferente los errores cometidos con las muestras de cada tipo de clase, penalizando en mayor medida los errores cometidos con muestras de la clase minoritaria frente a los errores cometidos con muestras de la clase mayoritaria.''')
...
Una métrica que nos permite evaluar dicho equilibrio es “sklearn.metrics.average_precision_score”, que calcula una aproximación del área bajo la curva precisión-sensibilidad (Area Under the Precision-Recall Curve, AUPRC en inglés), métrica que en casos de datos desbalanceados suele considerarse más estable que el área bajo la curva Característica Operativa del Receptor (Area Under the Receiver Operating Characteristic Curve, ROC-AUC), que suele usarse en clasificación binaria de conjuntos de datos balanceados para evaluar la capacidad discriminativa del modelo entre clase positiva y negativa en función del umbral de decisión establecido.

También puede ocurrir que el nivel de penalización considerado para un falso positivo y un falso negativo sean diferentes, es decir, puede ocurrir que un falso negativo sea muy grave, pero no lo sea un falso positivo o falsa alarma, en cuyo caso nos convendría usar otras métricas para potenciar minimizar por ejemplo la sensibilidad o recall del modelo para la clase minoritaria o modificar el umbral de decisión.

En este caso consideraremos que ambas situaciones (falso positivo y falso negativo), son igualmente perjudiciales y usaremos como parámetro de evaluación para GridSearchCV el citado “average_precision_score”, para lo cual crearemos nuestra propia métrica haciendo uso de “sklearn.metrics.make_scorer”.''')
