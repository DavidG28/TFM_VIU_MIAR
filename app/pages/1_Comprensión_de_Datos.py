...
with tab1:
    ...
    st.write('La descripción de los diferentes atributos sería:')
...
with tab2:
    ...
    num_ausentes = filtered_df.loc[:, ~filtered_df.columns.isin(['crisisJST', 'crisisJST_old'])].isnull().sum()
...
**A modo de ejemplo: el precio de la vivienda a lo largo del tiempo ha ido creciendo, pero ¿se podría decir de forma categórica si el precio actual es alto o no sin tomar ningún otro indicador como referencia? En principio sería complicado, pero si por ejemplo se calcula un crecimiento interanual de dicho precio en un 100%, sí se podría indicar que se ha producido un acusado aumento de los precios.**

El listado de atributos creados vendrá dado por:</div>''', unsafe_allow_html=True)
...
Nuevamente calculado a un plazo de dos años. Elevados volúmenes de crédito con expectativas de altos tipos de interés a largo plazo pueden indicar problemas futuros de salud financiera del país.''', unsafe_allow_html=True)
    ...
    with st.expander('+ :orange[Aumento de Crédito Global] - (global_loan2)'):
        st.markdown('''<div style="text-align: justify;">
...
Cálculo a dos años. En este caso se compara la evolución del nivel de crédito mundial excluyendo el país bajo estudio.''', unsafe_allow_html=True)
    ...
    with st.expander('+ :orange[Curva de Rendimiento Global (Global Yield Curve)] - (global_drate)'):
        st.markdown('''<div style="text-align: justify;">
...
La curva refleja la variación del rendimiento (tipo de interés), de la deuda pública emitida (bonos del estado), de manera global frente al país bajo estudio. Países en situación de mala salud financiera tendrán mayores problemas de financiación y deberán proporcionar mayores tipos de interés a los inversores para acceder a financiación externa.''', unsafe_allow_html=True)
    ...
    with st.expander('+ :orange[Ratio Tipos de Interes a Largo Plazo vs Corto Plazo] - (drate)'):
        st.markdown('''<div style="text-align: justify;">
...
with tab4:
    ...
    st.write('&emsp;Balance de Clases (crisisJST):&emsp;', 'No Crisis: ', round(data_final[data_final['crisisJST'] == 0].shape[0] / data_final.shape[0] * 100, 2), '%&emsp;Pre-Crisis: ', round(data_final[data_final['crisisJST'] == 1].shape[0] / data_final.shape[0] * 100, 2), '%', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col2:
